...
#     Revision 1.11  2011/04/26 22:41:44  rhubley
...
sub getParameters {
       ...
       && $value =~ /\d+/ )
  {
    $parameters .=
        ...
        . " -xdrop_gap "
        . int( $value / 2 ) . " ";
    $parameters .= " -min_raw_gapped_score $value -dust no ";
...
sub search {
  ...
  my $errFile;
  do {
    $errFile = $outputDirName . "/ncResults-" . time() . "-$$.err";
  } while ( -f $errFile );
  my $pid;
  ...
  if ( $DEBUG ) {
    ...
    my $outFile;
    do {
      $outFile = $outputDirName . "/ncResults-" . time() . "-$$.out";
    } while ( -f $outFile );
    open OUT, ">$outFile";
    while ( <$POUTPUT> ) {
      print OUT $_;
