...
#     Revision 1.109  2011/04/26 22:41:44  rhubley
...
sub search {
  ...
  $parseParams{'scoreMatrix'} = $this->getMatrix()
...
sub parseOutput {
  ...
  # Use of parameter 'scoreMatrix' indicates that we want to
  # replace the WUBlast score with the complexity adjusted score.
  my ( $matrixRef, $matrixAlphabet, $matrixFreqRef ) = ();
  my $value;
  my $lambda;
  my $scoreMatrixName = "";
  if ( defined( $value = $nameValueParams{'scoreMatrix'} ) ) {

    # Remove the path from the matrix file
    my @path = split( /[\\\/]/, $value );
    $scoreMatrixName = $path[ $#path ];
    $scoreMatrixName =~ s/[\n\r]//;

    # Test if matrix exists
    if ( -f $value ) {
      ( $matrixRef, $matrixAlphabet, $matrixFreqRef ) = _readMatrix( $value );
      $lambda = _calculateLambda( $matrixRef, $matrixFreqRef );

      # We need a defined matrix in order to calculate the
      # complexity adjusted score.  If the user overrides
      # the parameters then we cannot be sure which matrix was used.
      if ( !defined $matrixRef ) {
        croak $CLASS
            . "::parseOutput: Evidently you overrode the parameters to "
            . "this SearchEngine but tried to use "
            . "complexityAdjustedScoreMode bad bad bad!\n";
      }
    }
    else {
      croak $CLASS. "::parseOutput: Error...matrix ($value) does not exist!\n";
    }
  }
  ...
  while ( <$WUFILE> ) {
    ...
    #        to set the matrix from the scoreMatrix input...which
    #        is only used when we want to complexity adjust the
    #        scores.
    #
    #if ( /matrix=\s*(\S+)/ ) {
    #  my @path = split( /[\\\/]/, $1 );
